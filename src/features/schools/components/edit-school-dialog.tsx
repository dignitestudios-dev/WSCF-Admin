...
export function EditSchoolDialog({ open, onOpenChange, school }: EditSchoolDialogProps) {
  ...
  const onSubmit = async (data: SchoolFormData) => {
    if (!school) return;
    try {
      await updateSchool({ id: school._id, data });
      onOpenChange(false);
    } catch (error) {
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6 px-8 pb-8 pt-6">
          <div className="flex flex-col gap-2">
            <Label htmlFor="edit-name" className="font-poppins font-medium text-[14px] text-[#181818]">
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="edit-address" className="font-poppins font-medium text-[14px] text-[#181818]">
